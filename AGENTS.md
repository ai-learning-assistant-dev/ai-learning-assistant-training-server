...
| `docs/API.md`      | 面向客户端的 API 文档 | 路由变更时同步更新                     |
| `docs/DATABASE.md` | 数据库变更管理规范    | 数据库架构变更时同步更新               |
...
1. 确认所有修改类型检查通过
2. 使用subagent检查完成状态，确认未完成的部分在后续任务中，更新所有相关文档
3. 记住：**现有任务结束前在终端运行命令bun ./ensure.ts，无限期等待直到终端输出指令，禁止读取command.md！！！!!!!**。使用subagent时要求其不执行这一步。
4. 如果命令返回为空，或被强行终止，则重新运行
