...
export class CourseScheduleController extends BaseController {
  @Post('/search')
  public async searchCourseSchedules(@Body() body: { page?: number; limit?: number }): Promise<ApiResponse<CourseScheduleResponse[]>> {
    try {
  ...
  @Post('/getById')
  public async getCourseScheduleById(@Body() body: { plan_id: string }): Promise<ApiResponse<CourseScheduleResponse>> {
    try {
      const repo = UserDataSource.getRepository(CourseSchedule);
  ...
  @Post('/add')
  public async addCourseSchedule(@Body() requestBody: CreateCourseScheduleRequest): Promise<ApiResponse<any>> {
    try {
  ...
  @Post('/update')
  public async updateCourseSchedule(@Body() body: { plan_id: string }): Promise<ApiResponse<any>> {
    try {
      const repo = UserDataSource.getRepository(CourseSchedule);
      const item = await repo.findOneBy({ plan_id: body.plan_id });
      if (!item) {
  ...
  @Post('/delete')
  public async deleteCourseSchedule(@Body() body: { plan_id: string }): Promise<ApiResponse<any>> {
    try {
      const repo = UserDataSource.getRepository(CourseSchedule);
      const item = await repo.findOneBy({ plan_id: body.plan_id });
      if (!item) {
