...
export class ExerciseController extends BaseController {
  @Post('/search')
  public async searchExercises(@Body() body: { page?: number; limit?: number }): Promise<ApiResponse<ExerciseResponse[]>> {
    try {
  ...
  @Post('/getById')
  public async getExerciseById(@Body() body: { exercise_id: string }): Promise<ApiResponse<ExerciseResponse>> {
    try {
      const repo = MainDataSource.getRepository(Exercise);
  ...
  @Post('/add')
  public async addExercise(@Body() requestBody: CreateExerciseRequest): Promise<ApiResponse<any>> {
    try {
  ...
  @Post('/update')
  public async updateExercise(@Body() requestBody: UpdateExerciseRequest): Promise<ApiResponse<any>> {
    try {
  ...
  public async getExercisesWithOptionsBySection(@Body() body: { section_id: string }): Promise<ApiResponse<any[]>> {
    try {
      if (!body.section_id) {
  ...
  @Post('/delete')
  public async deleteExercise(@Body() body: { exercise_id: string }): Promise<ApiResponse<any>> {
    try {
      const repo = MainDataSource.getRepository(Exercise);
      const item = await repo.findOneBy({ exercise_id: body.exercise_id });
      if (!item) {
