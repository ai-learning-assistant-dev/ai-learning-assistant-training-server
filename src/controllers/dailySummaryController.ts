...
`;
      ...
      // 2. 获取练习结果
      const exerciseRepo = AppDataSource.getRepository(ExerciseResult);
      ...
      if (exerciseResults.length > 0) {
        const avgScore = exerciseResults.reduce((sum, r) => sum + (r.score || 0), 0) / exerciseResults.length;
        exerciseResultsText = `
完成练习数：${exerciseResults.length}
平均得分：${avgScore.toFixed(1)}分
练习详情：
${exerciseResults.map((r, i) => `${i + 1}. 得分：${r.score || 0}分\n   AI反馈：${r.ai_feedback || '无'}`).join('\n')}
`;
      }
