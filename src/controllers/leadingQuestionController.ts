...
export class LeadingQuestionController extends BaseController {
  ...
  public async searchLeadingQuestionsBySection(@Body() body: { section_id: string; page?: number; limit?: number }): Promise<ApiResponse<LeadingQuestionResponse[]>> {
    try {
  ...
  @Post('/search')
  public async searchLeadingQuestions(@Body() body: { page?: number; limit?: number }): Promise<ApiResponse<LeadingQuestionResponse[]>> {
    try {
      const pageNum = body.page || 1;
      const limitNum = body.limit || 10;
      const offset = (pageNum - 1) * limitNum;
  ...
  @Post('/getById')
  public async getLeadingQuestionById(@Body() body: { question_id: string }): Promise<ApiResponse<LeadingQuestionResponse>> {
    try {
      const repo = MainDataSource.getRepository(LeadingQuestion);
  ...
  @Post('/add')
  public async addLeadingQuestion(@Body() requestBody: CreateLeadingQuestionRequest): Promise<ApiResponse<any>> {
    try {
  ...
  @Post('/update')
  public async updateLeadingQuestion(@Body() requestBody: UpdateLeadingQuestionRequest): Promise<ApiResponse<any>> {
    try {
  ...
  @Post('/delete')
  public async deleteLeadingQuestion(@Body() body: { question_id: string }): Promise<ApiResponse<any>> {
    try {
      const repo = MainDataSource.getRepository(LeadingQuestion);
      const item = await repo.findOneBy({ question_id: body.question_id });
      if (!item) {
