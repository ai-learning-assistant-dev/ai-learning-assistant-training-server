...
export class AiInteractionController extends BaseController {
  @Post('/search')
  public async searchAiInteractions(@Body() body: { page?: number; limit?: number }): Promise<ApiResponse<AiInteractionResponse[]>> {
    try {
  ...
  @Post('/getById')
  public async getAiInteractionById(@Body() body: { interaction_id: string }): Promise<ApiResponse<AiInteractionResponse>> {
    try {
      const repo = UserDataSource.getRepository(AiInteraction);
  ...
  @Post('/add')
  public async addAiInteraction(@Body() requestBody: CreateAiInteractionRequest): Promise<ApiResponse<any>> {
    try {
  ...
  @Post('/update')
  public async updateAiInteraction(@Body() requestBody: UpdateAiInteractionRequest): Promise<ApiResponse<any>> {
    try {
  ...
  @Post('/delete')
  public async deleteAiInteraction(@Body() body: { interaction_id: string }): Promise<ApiResponse<any>> {
    try {
      const repo = UserDataSource.getRepository(AiInteraction);
      const item = await repo.findOneBy({ interaction_id: body.interaction_id });
      if (!item) {
