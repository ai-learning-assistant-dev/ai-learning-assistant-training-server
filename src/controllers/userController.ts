...
export class UserController extends BaseController {
  ...
  public async getChaptersAndSectionsByuser_id(@Body() body: { user_id: string }): Promise<ApiResponse<any>> {
    try {
      // 1. 查课程安排
   ...
   */
  @Post('/testJoinById')
  public async testJoinById(@Body() body: { user_id: string }): Promise<ApiResponse<any>> {
    try {
      // 使用原生 SQL 连表查询
      // 请根据实际表结构调整 SQL
      const sql = `
   ...
   */
  @Post('/getById')
  public async getUserById(@Body() body: { user_id: string }): Promise<ApiResponse<any>> {
    try {
      const repo = UserDataSource.getRepository(User);
   ...
   */
  @Post('/search')
  public async searchUsers(@Body() body: { name?: string; page?: number; limit?: number }): Promise<ApiResponse<UserResponse[]>> {
    try {
   ...
   */
  @Post('/add')
  public async addUser(@Body() requestBody: CreateUserRequest): Promise<ApiResponse<any>> {
    try {
      if (!requestBody.name) {
   ...
   */
  @Post('/update')
  public async updateUser(@Body() requestBody: UpdateUserRequest): Promise<ApiResponse<any>> {
    try {
   ...
   */
  @Post('/delete')
  public async deleteUser(@Body() body: { user_id: string }): Promise<ApiResponse<any>> {
    try {
      const repo = UserDataSource.getRepository(User);
      const user = await repo.findOneBy({ user_id: body.user_id });
      if (!user) {
