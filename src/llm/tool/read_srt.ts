...
function parseTimestamp(value: string): number | null {
  const match = value.match(/^(?<hh>\d{2}):(?<mm>\d{2}):(?<ss>\d{2}),(?<ms>\d{3})$/);
...
function sliceContext(items: NormalizedSrtItem[], centerIndex: number, window: number) {
...
export function getLinesAtTimestamp(source: string | SRTItem[], timestamp: string): JsonRecord {
  try {
    ...
    while (left <= right) {
      ...
      const { contextSlice, startSeq, endSeq } = sliceContext(items, mid, SRT_INIT_WINDOW);
...
export function readPreviousLines(source: string | SRTItem[], seq: number): JsonRecord {
  try {
    const items = normalizeSrtItems(source);
    if (items.length === 0) {
      return {
...
export function readNextLines(source: string | SRTItem[], seq: number): JsonRecord {
  try {
    const items = normalizeSrtItems(source);
    if (items.length === 0) {
      return {
