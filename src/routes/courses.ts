...
import { createCourseSchema, updateCourseSchema, importCourseSchema, courseChaptersSectionsRequestSchema } from '@schemas/course';
...
app.post(
  ...
  async c => {
    ...
    try {
      ...
      logger.error(`[courses] 课程导入失败 (title=${payloadTitle}):`, err);
      return c.json(fail('课程导入失败'), 500);
    }
  },
);
