...
import { createTestSchema, updateTestSchema, getTestsByCourseSchema, saveTestResultsSchema } from '@schemas/test';
...
app.post(
  '/getTestsWithExercisesByCourse',
  describeRoute({
    tags: ['练习与测试'],
    summary: '根据课程 ID 查询所有测试及其关联的练习题与选项',
  }),
  async c => {
    let course_id: string | undefined;
    try {
      ({ course_id } = getTestsByCourseSchema.parse(await c.req.json()));
      ...
      logger.error(`[tests] 查询课程测试题失败 (course_id=${course_id}):`, err);
      return c.json(fail('查询课程测试题失败'), 500);
    }
  },
);
...
app.post(
  ...
  describeRoute({
    tags: ['练习与测试'],
    summary: '批量保存测试答题结果，自动判分（选择题/简答题），计算总分与通过率',
  }),
  async c => {
    let user_id: string | undefined, test_id: string | undefined;
    try {
      const body = await c.req.json();
      const parsed = saveTestResultsSchema.parse(body);
      ({ user_id, test_id } = parsed);
      const { start_date, end_date, list } = parsed;
      const { ai_feedback } = body;
      ...
      logger.error(`[tests] 保存测试结果失败 (user_id=${user_id}, test_id=${test_id}):`, err);
      return c.json(fail('保存测试结果失败'), 500);
    }
  },
);
