...
import { createExerciseResultSchema, updateExerciseResultSchema } from '@schemas/exerciseResult';
...
app.post('/saveExerciseResults', async c => {
  ...
  if (!user_id || !list || !Array.isArray(list) || list.length === 0) {
    return c.json(fail('user_id 和 list 必须传，且 list 为非空数组'), 400);
  }
  ...
  if (pass && currentChapter && currentSection) {
    const unlockRow = await userDb
      ...
      .where(
        and(eq(userSectionUnlocks.user_id, user_id), eq(userSectionUnlocks.chapter_id, currentChapter.chapter_id), eq(userSectionUnlocks.section_id, currentSection.section_id)),
      )
      .limit(1);
